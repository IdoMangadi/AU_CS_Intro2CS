...
public class Point2D{
	...
	/**
	 * This method uses a simple calculating of a point in respect of the given point and scale it by given ratio:
	 */
	public void scale(Point2D cen, double ratio) {
		this._x = _x + (cen.x()-_x)*(1-ratio);
		this._y = _y + (cen.y()-_y)*(1-ratio);
	}
	/**
	 * This method compute the place of a point rotated by the given angle in respect of the given "center". 
	 * In order to get to the specific formula of this calculate, I used "https://stackoverflow.com/questions/36562488/2d-point-rotation-based-off-a-matrix"
	 */
	public void rotate(Point2D cen, double angleDegrees) {
		...
		this._x = cen.x() + x * Math.cos(radAngle) - y * Math.sin(radAngle);
		this._y = cen.y() + x * Math.sin(radAngle) + y * Math.cos(radAngle);
	}
